...
public class DormRoomImpl extends ServiceImpl<DormRoomMapper, DormRoom> implements DormRoomService {
     ...
     */
    @Override
    public int NewAddStudent(Student student, int RoomId, int bedId) {
        //调宿人
        String username = student.getUsername();
        //目标房间号
        int towardsRoomId = RoomId;
        //目标床号
        String towardsBedName = JudgeBedName.getBedName(bedId);
        ...
        //查询目标房间
        uw.eq("dormroom_id", towardsRoomId);
        //修改宿主名字
        uw.set(towardsBedName, username);
        //更改住房人数
        uw.set("current_capacity", currentCapacity2 + 1);
        //调用mapper层更新数据
    ...
    public Long selectHaveRoomStuNum() {
        ...
        Long first_bed = Long.valueOf(dormRoomMapper.selectCount(qw1));

        QueryWrapper<DormRoom> qw2 = new QueryWrapper<>();
        qw2.isNotNull("second_bed");
        Long second_bed = Long.valueOf(dormRoomMapper.selectCount(qw2));

        QueryWrapper<DormRoom> qw3 = new QueryWrapper<>();
        qw3.isNotNull("third_bed");
        Long third_bed = Long.valueOf(dormRoomMapper.selectCount(qw3));

        QueryWrapper<DormRoom> qw4 = new QueryWrapper<>();
        qw4.isNotNull("fourth_bed");
        Long fourth_bed = Long.valueOf(dormRoomMapper.selectCount(qw4));
    ...
    public int adjustRoomUpdate(AdjustRoom adjustRoom) {
        ...
        UpdateWrapper<DormRoom> uw1 = new UpdateWrapper<DormRoom>();
        ...
        if (result1 == 1) {
            UpdateWrapper<DormRoom> uw2 = new UpdateWrapper<DormRoom>();
    ...
    public DormRoom checkRoomState(Integer dormRoomId) {
        QueryWrapper<DormRoom> qw = new QueryWrapper<DormRoom>();
